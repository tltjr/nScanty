...
After forking and cloning the project, look in the app settings of web.config and change the settings to fit your needs. Don't forget to set a username and password, they will be required for all administrative functions.

There are two hard-coded connection strings at the moment. One in data\PostRepository and data\UserRepository. These will work fine if you have an instance of MongoDb (http://www.mongodb.org/) running locally (it is extraordinarily easy to setup).

The blog can also easily be used with App Harbor and App Harbor's MongoHQ add-on. To use MongoHQ, simply replace the local connection string with "var connection = ConfigurationManager.AppSettings.Get("MONGOHQ_URL");" and enable the add-on within
App Harbor. Upon deploy you'll be fully up and running.  
...
Feel free to email me with any questions: thomas.thornton.jr@gmail.com
